...
export const getAllMetals = asyncHandler(async (req, res) => {
  const result = await pool.query(`SELECT * FROM exchange.metals ORDER BY type ASC`);
  res.status(200).json(result.rows);
});

export const getAllMints = asyncHandler(async (req, res) => {
  const result = await pool.query(`SELECT * FROM exchange.mints ORDER BY name ASC`);
  res.status(200).json(result.rows);
});

export const getAllTypes = asyncHandler(async (req, res) => {
  const result = await pool.query(`SELECT DISTINCT product_type AS name FROM exchange.products`);
  res.status(200).json(result.rows);
});
