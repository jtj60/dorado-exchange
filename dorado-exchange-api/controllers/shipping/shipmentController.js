...
const getReturnShipment = async (req, res) => {
  const { order_id } = req.query;

  try {
    const query = `
      SELECT * FROM exchange.return_shipments WHERE order_id = $1;
    `;
    const values = [order_id];
...
const getReturnShipmentTracking = async (req, res) => {
  const { tracking_number, shipment_id, shipment_start, shipment_end } = req.body;
  ...
  try {
    ...
    const query = `
      ...
      FROM exchange.return_shipments s
      LEFT JOIN exchange.shipment_tracking_events e
        ON s.id = e.return_shipment_id
      WHERE s.id = $1
      GROUP BY s.id
    `;
    ...
    console.error("Error fetching return shipment tracking:", error);
    res.status(500).json({ error: "Failed to fetch tracking data." });
  }
};
