...
type PopoverContentProps = React.ComponentProps<typeof PopoverPrimitive.Content> & {
  /** Render inside the current DOM tree instead of portaling to body */
  portalled?: boolean
  /** If portalled, optionally portal into a specific container */
...
function PopoverHeader({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={cn(
        'flex w-full flex-col gap-1 rounded-t-md px-3 py-2',
        className,
      )}
      {...props}
    />
...
function PopoverFooter({ className, ...props }: React.ComponentProps<'div'>) {
  return (
    <div
      className={cn(
        'mt-auto grid w-full gap-2 rounded-b-md px-3 py-2',
        className,
      )}
      {...props}
    />
