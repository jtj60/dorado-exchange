...
import {
  ...
  useUpdateOfferNotes,
} from '@/features/orders/purchaseOrders/users/queries'
...
export default function RejectedPurchaseOrder({ order }: PurchaseOrderDrawerContentProps) {
  ...
  const handleUpdateOrderNotes = (offer_notes: string) => {
    updateOfferNotes.mutate({ purchase_order: order, offer_notes })
  }

  const carrierId = order.shipment?.carrier_id
  ...
  const returnInsurance: Insurance | undefined = useMemo(() => {
    return {
      insured: insureReturn,
      declaredValue: {
        amount: insureReturn ? declaredValue : 0,
        currency: 'USD',
      },
    }
  }, [insureReturn, declaredValue])
  ...
  const { data: rates = [], isLoading: ratesLoading } = useShippingRates(ratesInput)
        ...
        <div className="flex w-full mb-4">
          <div className="flex flex-col">
            ...
            <div className="text-sm text-neutral-600 text-left mb-4">
              Please consider leaving us a note as to why you rejected our initial offer. We will be
              in touch soon if we choose to update your offer.
            </div>
            ...
            <div className="flex flex-col gap-0 w-full">
              ...
              <Button
                ...
                onClick={() => handleUpdateOrderNotes(offerNotes)}
        ...
        <div className="flex flex-col h-full w-full gap-3 mb-4">
          ...
          {order.spots_locked ? (
            <p className="text-sm text-neutral-700 text-left">
              We have updated your offer. Spot prices have been locked for your order. You will have
              24 hours to accept or reject the new offer, after which we will unlock the current
              spot prices.
            </p>
          ) : (
            <p className="text-sm text-neutral-700 text-left">
              We have updated your offer. Spot prices have not been locked for your order. You will
              have 1 week to accept or reject the new offer, after which it will automatically be
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="bg-background">
            ...
            <DialogDescription>
              ...
              for return. Before your items are returned, you will need to pay the shipping charges,
              including the return shipment. Please select if you would like your return shipment
              insured.
            </DialogDescription>
            ...
            >
              {(['insured', 'uninsured'] as const).map((option) => {
            ...
            <div className="flex w-full justify-between items-center">
              ...
              {!ratesLoading && selectedRate?.netCharge != null ? (
            ...
            <div className="flex w-full justify-between items-center">
              ...
              {!ratesLoading && selectedRate?.netCharge != null ? (
                <PriceNumberFlow
                  value={(order.shipment?.shipping_charge ?? 0) + selectedRate.netCharge}
                />
              ) : (
                <span className="text-neutral-500 select-none">Loading...</span>
              )}
