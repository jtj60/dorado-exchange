...
export default function ProductCards({
  ...
  const metalOptions: {
  ...
  }[] = [
    {
      label: 'Gold',
      getLogo: (isSelected) => (
        <GoldIcon size={36} stroke={isSelected ? 'white' : getCustomPrimaryIconStroke()} />
      ),
    ...
    },
    {
      label: 'Silver',
      getLogo: (isSelected) => (
        <SilverIcon size={36} stroke={isSelected ? 'white' : getCustomPrimaryIconStroke()} />
      ),
    ...
    },
    {
      label: 'Platinum',
      getLogo: (isSelected) => (
        <PlatinumIcon size={36} stroke={isSelected ? 'white' : getCustomPrimaryIconStroke()} />
      ),
    ...
    },
    {
      label: 'Palladium',
      getLogo: (isSelected) => (
        <PalladiumIcon size={36} stroke={isSelected ? 'white' : getCustomPrimaryIconStroke()} />
      ),
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 w-full mb-6">
      {productsInventory &&
              ...
              <div className="flex flex-col w-full">
                <div className="flex items-center w-full justify-between">
                  <div className={cn('flex items-center gap-2', isSelected && 'text-white')}>
                    {icon}
                    <div className={cn('text-lg text-primary-gradient', isSelected && 'text-white')}>{metal}</div>
                  </div>
                  <div className={cn('text-xl', isSelected && 'text-white')}>
                    <PriceNumberFlow
                ...
                <div className="flex flex-col mt-2 gap-1">
                  {[
                    ...
                    <div key={i} className="flex items-center w-full justify-between">
                      <div
                        className={cn('text-sm', isSelected ? 'text-white' : 'text-neutral-600')}
                      >
                        {label}
                      </div>
                      <div
                        className={cn('text-sm', isSelected ? 'text-white' : 'text-neutral-800')}
                      >
                        {value}
                      </div>
                    </div>
